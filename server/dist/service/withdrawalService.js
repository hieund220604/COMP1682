...
exports.withdrawalService = {
    ...
    async resendOTP(withdrawalId, userEmail) {
        ...
        try {
            await emailService_1.emailService.sendOTPEmail(userEmail, otp);
        }
        catch (error) {
            console.error('Failed to send OTP email:', error);
    ...
    async verifyOTP(withdrawalId, otp) {
        // Get withdrawal
        const withdrawal = await prisma_1.prisma.withdrawal.findUnique({
            where: { id: withdrawalId },
            include: { user: true }
        ...
        const [updated, _] = await prisma_1.prisma.$transaction([
            prisma_1.prisma.withdrawal.update({
                where: { id: withdrawalId },
                data: {
                    status: 'COMPLETED',
                    verifiedAt: new Date(),
        ...
        const userUpdated = await prisma_1.prisma.user.findUnique({
            where: { id: withdrawal.userId }
        });
        return {
            success: true,
