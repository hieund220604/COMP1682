...
exports.debtController = {
    async getUserDebts(req, res) {
        try {
            ...
            const debts = await settlementService_1.settlementService.getUserDebts(userId, groupId);
    ...
    async payWithBalance(req, res) {
        try {
            ...
            const result = await settlementService_1.settlementService.payWithBalance(userId, settlementId);
    ...
    async payWithVNPay(req, res) {
        try {
            ...
            const ipAddr = req.headers['x-forwarded-for'] ||
                ...
                '127.0.0.1';
            const result = await vnpayService_1.vnpayService.createPaymentUrl(settlementId, returnUrl, ipAddr);
            res.json({ success: true, data: result });
        }
        catch (error) {
            res.status(400).json({
    ...
    async quickPay(req, res) {
        try {
            ...
            const result = await settlementService_1.settlementService.quickPayDebt(userId, groupId, toUserId, amount, method);
            if (method === 'VNPAY' && result.status === 'PENDING') {
                ...
                const vnpayResult = await vnpayService_1.vnpayService.createPaymentUrl(result.settlementId, returnUrl, ipAddr);
     ...
     */
    async getMyPendingDebts(req, res) {
        try {
            const userId = req.userId;
            const { groupId } = req.params;
            if (!userId) {
            ...
            const result = await settlementService_1.settlementService.getMyPendingDebts(userId, groupId);
            res.json({ success: true, data: result });
        }
        catch (error) {
            res.status(400).json({
     ...
     */
    async getMyPendingCredits(req, res) {
        try {
            const userId = req.userId;
            const { groupId } = req.params;
            if (!userId) {
            ...
            const result = await settlementService_1.settlementService.getMyPendingCredits(userId, groupId);
            res.json({ success: true, data: result });
        }
        catch (error) {
            res.status(400).json({
